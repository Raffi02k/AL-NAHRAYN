...
export default function GalleryPage() {
  return (
    <main>
      ...
      <section className="section gallery-page"><div className="shell gallery-grid">{gallery.map(([src, alt], index) => <figure className={`gallery-item gallery-item-${index + 1} ${src.includes("menu-board") ? "gallery-item-board" : ""}`} key={src}><img src={src} alt={alt} loading="lazy" decoding="async" /><figcaption>{String(index + 1).padStart(2, "0")} — {alt}</figcaption></figure>)}</div></section>
